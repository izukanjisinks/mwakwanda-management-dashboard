...
watch(() => props.open, (isOpen) => {
  if (isOpen) {
    existingImages.value = props.venue?.images ? [...props.venue.images] : []
    pending.value = []
  }
})
...
const hasChanges = computed(() => {
  const originalUrls = props.venue?.images ?? []
...
async function handleUpload() {
  if (!hasChanges.value || !props.venue) return
  uploading.value = true
  try {
    const files = pending.value.map(p => p.file)
    await store.uploadImages(props.venue.id, files, existingImages.value)
    toast.success(`Gallery updated for ${props.venue.name}.`)
    clearPending()
    emit('update:open', false)
  } catch (err) {
    console.error('[VenueImageDialog] upload error:', err)
  ...
  <Dialog :open="open" @update:open="handleClose">
    <DialogContent class="max-w-2xl p-0 gap-0 overflow-hidden rounded-2xl">
      ...
      <DialogHeader class="px-8 pt-8 pb-4 text-center border-b border-border/10">
        <DialogTitle class="text-2xl font-bold font-headline">Venue Images</DialogTitle>
        <DialogDescription class="text-sm text-muted-foreground mt-1">
          Manage photography for <span class="font-semibold text-foreground">{{ venue?.name ?? 'this venue' }}</span>
        </DialogDescription>
      </DialogHeader>
